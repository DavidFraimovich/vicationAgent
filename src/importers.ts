...
export function classifyPlace(name: string, address = ""): string[] {
...
export function importSavedPlaces(file: string, tripId: string): {
  imported: number;
  skipped: number;
  errors: string[];
} {
  ...
  for (const [index, feature] of features.entries()) {
    try {
      ...
      const url = properties.google_maps_url as string | undefined;
      const name =
...
export function importReviews(file: string, tripId: string): {
  imported: number;
  skipped: number;
  errors: string[];
} {
  ...
  for (const [index, feature] of features.entries()) {
    try {
      ...
      const coordinates = feature?.geometry?.coordinates ?? [];
      const url = properties.google_maps_url as string | undefined;
      const name = location.name as string | undefined;
      if (!name && !url) {
        skipped += 1;
        continue;
      }
