...
function normalizeTrip(row: any): JsonObject {
  return {
    id: row.id,
    name: row.name,
...
export function listTrips(): JsonObject[] {
  return (getDb().prepare("SELECT * FROM trips ORDER BY start_date, name").all() as any[]).map(normalizeTrip);
}

export function getTrip(id: string): JsonObject | undefined {
...
export function findPlaceByUrl(tripId: string, url?: string): JsonObject | undefined {
  if (!url) return undefined;
  const row = getDb().prepare(
    "SELECT * FROM places WHERE trip_id = ? AND google_maps_url = ? LIMIT 1"
  ).get(tripId, url) as any;
  return row ? normalizePlace(row) : undefined;
}

export function upsertPlace(input: PlaceInput): JsonObject {
  const db = getDb();
  const existingByUrl = input.id ? undefined : findPlaceByUrl(input.tripId, input.googleMapsUrl);
  const id = input.id ?? (existingByUrl?.id as string | undefined) ?? randomUUID();
  const t = now();
  ...
  `).run(
    ...
    input.priority ?? (existingByUrl?.priority as string | undefined) ?? "C",
    input.planningStatus ?? (existingByUrl?.planningStatus as string | undefined) ?? "discovered",
    input.notes ?? null,
    input.source ?? null,
    input.lastVerifiedAt ?? null,
    json(input.metadata ?? (existingByUrl?.metadata as JsonObject | undefined) ?? {}),
    t,
    t,
  );
  return getPlace(id)!;
...
function normalizePlace(row: any): JsonObject {
  return {
    id: row.id,
    tripId: row.trip_id,
...
export function getPlace(id: string): JsonObject | undefined {
  const row = getDb().prepare("SELECT * FROM places WHERE id = ?").get(id) as any;
  return row ? normalizePlace(row) : undefined;
}
...
export function listPlaces(input: {
  ...
  const params: Array<string | number | null> = [input.tripId];
  if (input.status) {
    clauses.push("planning_status = ?");
  ...
  if (input.query) {
    clauses.push("(name LIKE ? OR address LIKE ? OR notes LIKE ?)");
    const q = `%${input.query}%`;
    params.push(q, q, q);
  }
  ...
  const rows = getDb().prepare(`
    ...
    ORDER BY
      CASE priority WHEN 'A' THEN 1 WHEN 'B' THEN 2 WHEN 'C' THEN 3
...
export function setPlaceStatus(input: {
  ...
  const current = getPlace(input.id);
  if (!current) throw new Error(`Place not found: ${input.id}`);
  return upsertPlace({
    id: input.id,
    tripId: current.tripId as string,
    sourceId: current.sourceId as string | undefined,
    googlePlaceId: current.googlePlaceId as string | undefined,
    name: current.name as string,
    address: current.address as string | undefined,
    countryCode: current.countryCode as string | undefined,
    latitude: current.latitude as number | null,
    longitude: current.longitude as number | null,
    googleMapsUrl: current.googleMapsUrl as string | undefined,
    categories: input.categories ?? current.categories as string[],
    priority: input.priority ?? current.priority as string,
    planningStatus: input.planningStatus ?? current.planningStatus as string,
    notes: input.notes ?? current.notes as string | undefined,
    source: current.source as string | undefined,
    lastVerifiedAt: current.lastVerifiedAt as string | undefined,
    metadata: current.metadata as JsonObject,
  });
}
...
export function upsertItineraryItem(input: ItineraryInput): JsonObject {
  const db = getDb();
...
function normalizeItinerary(row: any): JsonObject {
  return {
    id: row.id,
    tripId: row.trip_id,
...
export function getItineraryItem(id: string): JsonObject | undefined {
  const row = getDb().prepare("SELECT * FROM itinerary_items WHERE id = ?").get(id) as any;
  return row ? normalizeItinerary(row) : undefined;
}

export function listItinerary(tripId: string, dayDate?: string): JsonObject[] {
  const rows = dayDate
    ? getDb().prepare(`
    ...
    : getDb().prepare(`
      ...
      `).all(tripId);
  return (rows as any[]).map(normalizeItinerary);
}

export function deleteItineraryItem(id: string): void {
...
export function upsertLodging(input: LodgingInput): JsonObject {
  const db = getDb();
  const id = input.id ?? randomUUID();
  const t = now();
  db.prepare(`
    ...
    ON CONFLICT(id) DO UPDATE SET
      ...
      updated_at=excluded.updated_at
  `).run(
    id, input.tripId, input.baseName ?? null, input.checkIn ?? null,
    input.checkOut ?? null, input.provider, input.title, input.url ?? null,
    input.totalPrice ?? null, input.currency ?? null, input.rating ?? null,
    input.latitude ?? null, input.longitude ?? null,
    input.parking === undefined ? null : Number(input.parking),
    input.cancellation ?? null, input.status ?? "candidate",
    input.notes ?? null, json(input.metadata ?? {}), t, t,
  );
  return getLodging(id)!;
}

function normalizeLodging(row: any): JsonObject {
  return {
    id: row.id,
    tripId: row.trip_id,
...
export function getLodging(id: string): JsonObject | undefined {
  const row = getDb().prepare("SELECT * FROM lodging_candidates WHERE id = ?").get(id) as any;
  return row ? normalizeLodging(row) : undefined;
}

export function listLodging(tripId: string): JsonObject[] {
  return (getDb().prepare(`
    SELECT * FROM lodging_candidates
    WHERE trip_id = ?
    ORDER BY base_name, status, total_price
  `).all(tripId) as any[]).map(normalizeLodging);
}
...
export function recordAction(input: {
  ...
  `).run(
    id, input.tripId ?? null, input.provider, input.action,
    input.entityType ?? null, input.entityId ?? null, input.status,
    Number(input.dryRun ?? true), json(input.payload ?? {}),
    json(input.result ?? {}), createdAt,
  );
  return {
...
export function listActions(tripId?: string, limit = 200): JsonObject[] {
  const rows = tripId
    ? getDb().prepare(`
...
export function getSetting<T>(key: string, fallback: T): T {
  const row = getDb().prepare("SELECT value_json FROM settings WHERE key = ?").get(key) as any;
...
export function recordImportRun(input: {
  ...
  `).run(
    randomUUID(), input.sourceFile, input.sourceType, input.tripId ?? null,
    input.importedCount, input.skippedCount, json(input.errors), now(),
  );
}
...
export function copyDatabase(destination: string): void {
  const source = dbPath();
