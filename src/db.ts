...
export function findPlaceByUrl(tripId: string, url?: string): JsonObject | undefined {
  ...
  const row = getDb().prepare(
    "SELECT * FROM places WHERE google_maps_url = ? LIMIT 1"
  ).get(url) as any;
...
export function upsertPlace(input: PlaceInput): JsonObject {
  ...
  if (input.linkToTrip !== false) {
    ...
    `).run(
      input.tripId,
      id,
      json(input.categories ?? (existingByUrl?.categories as string[] | undefined) ?? []),
      input.priority ?? (existingByUrl?.priority as string | undefined) ?? "C",
      input.planningStatus ?? (existingByUrl?.planningStatus as string | undefined) ?? "discovered",
      input.notes ?? null,
      t,
...
function normalizePlace(row: any): JsonObject {
  return {
    ...
    categories: parseJson<string[]>(row.trip_categories_json ?? row.categories_json, []),
...
export function getPlace(id: string, tripId?: string): JsonObject | undefined {
  const row = tripId
    ? getDb().prepare(`
      ...
      `).get(id, tripId) as any
    : getDb().prepare("SELECT * FROM places WHERE id = ?").get(id) as any;
  return row ? normalizePlace(row) : undefined;
}
...
export function setPlaceStatus(input: {
  ...
  `).run(
    json(input.categories ?? current.categories as string[]),
    input.priority ?? current.priority as string,
    input.planningStatus ?? current.planningStatus as string,
    input.notes ?? current.notes as string | null ?? null,
    now(),
    input.tripId,
    input.id,
  );
