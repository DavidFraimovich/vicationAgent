...
יש לבצע preview לפני שליחה. קיימת הרשאת שליחה קבועה, ולכן אין צורך באישור
נפרד לכל הודעה. הכלי שולח רק ליעד הקבוע בקובץ `.env`, ושומר ביומן הפעולות רק מטא־דאטה כגון אורך
...
לפני כל אחת מהפקודות האלה — כולל קריאה, preview, יצירה, עדכון או מחיקה —
חובה לקרוא וליישם את
`.agents/skills/travel-planner/references/calendar-policy.md`. זהו מקור האמת
התפעולי לפקודות Calendar, והוא גובר על תיאור מקוצר של כלי.

פעולות Calendar פועלות רק מול ה־Calendar ID המוגדר ב־
`config/travel-agent.yaml` עבור היומן `חו״ל`. כתיבה ומחיקה דורשות
`confirm: true`. לפני יצירה קוראים את טווח התאריכים למניעת כפילות; לפני
עדכון או מחיקה קוראים את האירוע המדויק לפי Event ID.

שדה תיאור האירוע הוא plain text קנוני התואם ל־Apple Calendar: ללא HTML,
Markdown או rich text, עם מעברי שורה אמיתיים, רשימות `- `, קישורי
`https://` מלאים וגלויים ומספרי טלפון גלויים בפורמט בינלאומי. אין לנקות או
לכתוב תיאור דרך עורך Google Calendar, פעולת `הסרת העיצוב` או Apple Calendar.

יצירה ועדכון מתבצעים ישירות דרך Google Calendar API; Advanced Calendar
Service של Apps Script הוא fallback כאשר המחבר המקומי אינו יכול לכתוב או
לאמת מידע גולמי. מתקנים אירוע במקום לפי Event ID ולא מוחקים ויוצרים מחדש
רק כדי לתקן תיאור, תגיות או מוזמנים.

באירועי המסלול:

- האירוע קיים רק ביומן `חו״ל`;
- `davidfr97@gmail.com` אינו מוזמן ומוסר מאירוע קיים;
- מוזמנים אחרים נשמרים;
- ברירת המחדל היא `sendUpdates: none`.

אחרי יצירה או עדכון מבצעים קריאת API גולמית ומוודאים התאמה מדויקת לתיאור
הקנוני, היעדר `<p>`,‏ `<br>` וכל תג HTML, שמירת URLs מלאים והיעדר המוזמן
האישי. אחרי מחיקה מוודאים שה־Event ID אינו קיים. רק לאחר האימות מתעדים
הצלחה ב־audit.
