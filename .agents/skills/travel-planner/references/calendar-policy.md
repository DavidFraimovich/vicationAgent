...
- שעות פעילות בתאריך שלנו: [שעות + מקור; או "פתוח לציבור" ומגבלת גישה אמיתית]
...
שעות הפעילות אינן הערת רשות. לכל עסק, מתקן, רכבל, אטרקציה,
השכרה, מסעדה או חניה בתשלום שנכנסים ללוח הזמנים יש לרשום את
השעות הרלוונטיות לתאריך הביקור ואת מקור האימות. אם השעות אינן
מאומתות, כותבים זאת במפורש ולא בונים עליהן רצף קריטי. במקום טבע
פתוח מציינים `פתוח לציבור` ואת המגבלה המעשית: אור יום, מזג אוויר,
כביש, חניה או הזמנה.
...
17:00–18:30 פעילות קלה/שיטוט/קפה/ארוחה מוקדמת על הציר
19:00–20:30 ערב
```

לא להכניס 3 מסלולים ביום אחד.

ביום פעיל המסלול חייב להישאר רציף מחוץ ללינה: יעד → יעד → פעילות
קלה/אוכל → הפעילות או הארוחה האחרונה → לינה. אין ליצור אירוע חזרה לעוגן
באמצע היום לצורך מנוחה, ישיבה, החלפת בגדים, מקלחת, טעינת ציוד או buffer,
ואז אירוע יציאה נוסף. חזרה לפני סוף היום מותרת רק לפי בקשה מפורשת או בגלל
אילוץ בטיחותי, רפואי, מזג אוויר חמור או לוגיסטיקה שאין לה חלופה.

מקלחת אישית מתוכננת רק בלילה, אחרי הפעילות או הארוחה האחרונה של היום. אין
להשתמש בחלון חזרה ללינה באמצע אחר הצהריים כעצירת מקלחת לפני יציאה נוספת.
