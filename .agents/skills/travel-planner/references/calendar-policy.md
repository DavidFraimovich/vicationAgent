...
language: he
version: Codex split v9 — mandatory raw Calendar API workflow for every calendar command
...
זהו קובץ המדיניות המחייב לכל פעולת Calendar בפרויקט. לפני קריאה תפעולית,
preview, יצירה, עדכון, עריכה או מחיקה של אירוע, בכל כלי או ממשק, חייבים לקרוא
וליישם את הקובץ הזה. תיאור של פקודה או כלי אינו מחליף את המדיניות; במקרה של
סתירה, הקובץ הזה גובר.
...
## 2א. חוזה ביצוע מחייב לכל פקודת Calendar

הכללים הבאים חלים על `calendar_list_events`,‏ `calendar_preview_event`,‏
`calendar_apply_event`,‏ `calendar_delete_event`, על Google Calendar connector,
על Google Calendar API, על Advanced Calendar Service של Apps Script ועל כל
פעולת Calendar דרך דפדפן:

1. לקרוא את הקובץ הזה לפני הפקודה ולבדוק שה־Calendar ID הוא ה־ID של `חו״ל`.
2. לקרוא את המצב הקיים לפני שינוי: אירוע לפי ID בעדכון/מחיקה, וטווח תאריכים
   רלוונטי לפני יצירה כדי למנוע כפילות.
3. להציג preview או diff לפני כתיבה, ולדרוש את האישור שמוגדר בפרויקט.
4. להעדיף עדכון במקום באמצעות Event ID קיים. אין למחוק וליצור מחדש אוסף
   אירועים כדי לתקן תיאור, מוזמן או עיצוב, אלא אם המשתמש אישר זאת במפורש
   והוכח שאין דרך בטוחה לעדכן במקום.
5. כתיבה מתבצעת ישירות דרך Google Calendar API. אם מחבר
   `travel_local` אינו זמין או אינו חושף את הערך הגולמי, מותר להשתמש
   ב־Advanced Calendar Service של Apps Script. אין להשתמש בעורך התיאור העשיר
   של Google Calendar או ב־Apple Calendar לצורך כתיבה/ניקוי.
6. בכל יצירה או עדכון יש להשתמש ב־`sendUpdates: none`, אלא אם המשתמש ביקש
   במפורש לשלוח הודעות למוזמנים.
7. אירועי המסלול נשמרים ביומן `חו״ל` בלבד. אין להוסיף את
   `davidfr97@gmail.com` כמוזמן; בעדכון אירוע קיים יש להסיר כתובת זו
   מ־`attendees` ולשמר מוזמנים אחרים. המשתמש נתן לכך אישור מפורש בתאריך
   08.08.2026.
8. תיאור נשלח כ־plain text קנוני עם מעברי שורה אמיתיים, `- ` לרשימות,
   כתובות `https://` מלאות וגלויות ומספרי טלפון גלויים. אסור לשלוח HTML,
   Markdown, rich text, `<p>` או `<br>`.
9. אחרי כל יצירה או עדכון יש לבצע `events.get` גולמי ולוודא:
   - האירוע נמצא ביומן `חו״ל`;
   - התיאור זהה בדיוק ל־plain text הקנוני;
   - אין התאמה ל־`<[^>]+>`;
   - ה־URLs המלאים נשמרו;
   - `davidfr97@gmail.com` אינו מוזמן באירוע מסלול.
10. אחרי מחיקה יש לקרוא מחדש לפי ה־ID ולאמת שהאירוע אינו קיים ביומן היעד.
11. לתעד כל כתיבה או מחיקה ב־audit המקומי, כולל מספר האירועים, תוצאת
    האימות, `sendUpdates`, כשלונות ו־dry-run.

Apple Calendar הוא לקוח תצוגה המסונכרן מ־Google. מותר להשתמש בו לבדיקת
קריאות וקישורים לאחר הסנכרון, אך הוא אינו מקור האמת לכתיבה ואינו תחליף
לאימות גולמי דרך Calendar API.

## 2ב. לינות Airbnb — מקור מהמייל ומניעת כפילויות
...
- אם קיים אירוע שנוצר מהמייל או מהספק ביומן `חו״ל`, לא יוצרים אירוע נוסף.
  מותר לעדכן אותו במקום רק כדי להוסיף פרטים רלוונטיים שחסרים בו, תוך שמירת
  תוכן קיים ומוזמנים אחרים. אין לערוך אירוע Airbnb שנמצא ביומן אחר.
- אין למחוק אירוע Airbnb שיובא מהמייל או מהספק. הרשאת העריכה אינה הרשאת
  מחיקה.
...
- יש לכתוב את התיאור ישירות דרך Google Calendar API או דרך Advanced Calendar
  Service של Apps Script. אין להשתמש בעורך התיאור העשיר של Google Calendar
  לצורך ניקוי, משום שגם `הסרת העיצוב` עלולה לשמור מעברי שורה כתגיות `<br>`.
- אחרי הכתיבה יש לקרוא את האירוע מחדש דרך אותו API גולמי ולוודא שהתיאור זהה
  בדיוק ל־plain text הקנוני, שאין בו התאמה ל־`<[^>]+>`, ושכל URL מלא נשמר.
  קריאה רגילה דרך מחבר שמנרמל תיאורים אינה מספיקה לאימות.
- אירועי המסלול צריכים להתקיים ביומן `חו״ל` בלבד, ללא
  `davidfr97@gmail.com` כמוזמן. בעת תיקון אירוע קיים יש להסיר כתובת זו
  ממערך `attendees` ולכתוב עם `sendUpdates: none`, כדי לא לשלוח התראות
  ביטול. אין להסיר מוזמנים אחרים בלי בקשה מפורשת.
...
1. לקרוא וליישם את חוזה הביצוע בסעיף 2א.
2. לוודא שהוא לא קיים כבר ביומן באמצעות קריאת טווח התאריכים הרלוונטי.
3. לוודא מיקום ב־Google Maps.
4. לוודא שעה הגיונית מול נסיעות.
5. לבדוק אם המקום צריך רכבל/חניה/כרטיס.
6. להכניס תיאור לפי התבנית.
7. לוודא שהתיאור הוא plain text בלבד, ללא HTML או Markdown, ושהקישורים ומספרי הטלפון גלויים.
8. להוסיף fallback.
9. לא ליצור אירוע Conditional אם אין החלטה שמתאימה לאותו יום.
10. אם זה אירוע לינה, לעצור כברירת מחדל: לבדוק את מייל האישור בחשבון
    `david04031997@gmail.com` ואת טווח הלינה ביומן. ליצור ידנית רק אם אין
    אירוע ספק והמשתמש ביקש זאת במפורש.
11. ליצור ישירות דרך Calendar API עם `sendUpdates: none`, ללא מוזמן אישי.
12. לקרוא את האירוע מחדש לפי ה־ID שהוחזר ולבצע את כל בדיקות האימות הגולמיות
    בסעיף 2א לפני דיווח הצלחה.

## 11. עדכון אירוע — פרוטוקול
כאשר מעדכנים אירוע:

1. לקרוא וליישם את חוזה הביצוע בסעיף 2א.
2. לקרוא את האירוע הקיים לפי Event ID ולשמור את ה־ID הקיים.
3. לשמר מידע ידני חשוב, מוזמנים אחרים ופרטי לוגיסטיקה שלא התבקשו להשתנות.
4. להוסיף/לתקן Google Maps link, “מה לראות / לעשות” והמלצה אם חסרים.
5. להמיר HTML קיים לטקסט פשוט תוך שמירת התוכן, הקישורים ומספרי הטלפון.
6. להסיר את `davidfr97@gmail.com` מ־`attendees` באירועי המסלול, בלי להסיר
   מוזמנים אחרים.
7. לבצע `events.patch` ישיר לפי ה־ID עם `sendUpdates: none`; אין להשתמש
   בעורך העשיר, ב־`הסרת העיצוב`, ב־Apple Calendar או במחיקה/יצירה מחדש.
8. לקרוא מחדש את האירוע דרך API גולמי ולאמת התאמה מלאה, היעדר HTML, URLs
   מלאים והיעדר המוזמן האישי.
9. אם האירוע נהיה לא מתאים — לסמן כ־Optional או להחליף, לא למחוק בלי אישור.

## 12. מחיקת אירוע — פרוטוקול
לפני מחיקה יש לקרוא וליישם את חוזה הביצוע בסעיף 2א. למחוק אירוע רק אם:

- המשתמש ביקש במפורש.
- זה אירוע שנוצר בטעות.
- זה כפול ברור.

יש לקרוא את האירוע המדויק לפי ID ולהציג preview עם כותרת/זמן/יומן. טיסה
מוגנת מסורבת תמיד. באירוע ספק/לינה עוצרים ודורשים אישור מפורש שמזהה את
האירוע המדויק; אין להסתפק באישור כללי לניקוי כפילויות.
המחיקה מתבצעת דרך Calendar API עם `sendUpdates: none` כאשר האפשרות זמינה.
אחריה יש לוודא בקריאה חוזרת שה־ID אינו קיים ביומן `חו״ל` ולתעד את הפעולה.
לא למחוק טיסות, לינה, אירועי ספק או אירועים עם מידע פרטי בלי אישור מפורש.
