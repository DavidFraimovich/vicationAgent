...
import { listActions, listItinerary, listLodging, listPlaces, listTrips, setPlaceStatus, upsertItineraryItem, upsertLodging, upsertPlace, } from "./db.js";
...
const server = http.createServer(async (req, res) => {
    try {
        ...
        if (req.method === "POST" && url.pathname === "/api/place/status") {
            ...
            sendJson(res, setPlaceStatus({
                id: input.placeId,
                tripId: input.tripId ?? config.project.default_trip_id,
                ...input,
            }));
            return;
        }
