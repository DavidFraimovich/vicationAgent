...
server.tool("travel_get_trip", "Get a trip and its aggregate state.", { tripId: z.string() }, async ({ tripId }) => result({
    trip: getTrip(tripId),
...
server.tool("travel_upsert_trip", "Create or update a local trip.", {
    id: z.string(),
    name: z.string(),
...
server.tool("travel_list_places", "List/filter local places for a trip.", {
    tripId: z.string(),
    status: z.string().optional(),
...
server.tool("travel_get_place", "Get one local place.", { placeId: z.string(), tripId: z.string().optional() }, async ({ placeId, tripId }) => result(getPlace(placeId, tripId)));
server.tool("travel_upsert_place", "Create or update a local place. Use after resolving with Google Maps.", {
...
server.tool("travel_set_place_status", "Update a place's priority, planning status, categories, or notes.", {
    placeId: z.string(),
    tripId: z.string(),
    planningStatus: z.string().optional(),
    priority: z.string().optional(),
...
server.tool("travel_list_itinerary", "List itinerary items, optionally for one day.", { tripId: z.string(), dayDate: z.string().optional() }, async ({ tripId, dayDate }) => result(listItinerary(tripId, dayDate)));
server.tool("travel_upsert_itinerary_item", "Create or update a local itinerary item before Calendar publication.", {
    id: z.string().optional(),
    tripId: z.string(),
    dayDate: z.string(),
...
server.tool("travel_validate_trip", "Validate unresolved places, itinerary descriptions, lunch windows, and overload.", { tripId: z.string() }, async ({ tripId }) => result(validateTrip(tripId)));
server.tool("travel_export_mymaps_csv", "Export trip places to a Google My Maps compatible CSV.", { tripId: z.string(), outputFile: z.string().optional() }, async ({ tripId, outputFile }) => result(exportMyMapsCsv(tripId, outputFile)));
server.tool("lodging_build_airbnb_search_url", "Build an Airbnb search URL for use in the signed-in Chrome profile.", {
    location: z.string(),
    checkIn: z.string(),
    checkOut: z.string(),
...
server.tool("lodging_list_candidates", "List lodging candidates.", { tripId: z.string() }, async ({ tripId }) => result(listLodging(tripId)));
server.tool("travel_record_external_action", "Record a Google Maps, Chrome, Airbnb, Booking, or other external action.", {
...
server.tool("calendar_apply_event", `Create or update an event in the allowlisted calendar. Read and follow ${CALENDAR_POLICY_FILE} before use. Update in place by Event ID, keep itinerary events only in חו״ל, remove the personal-calendar attendee, use sendUpdates=none, and verify the raw stored event after writing. Descriptions are canonical Apple Calendar-compatible plain text. confirm=true is required by policy.`, {
    ...
    itineraryItemId: z.string().optional(),
    eventId: z.string().optional(),
    allowLodgingWriteAfterDedupe: z.boolean().optional().describe("Use only after the user explicitly requests a manual lodging event and the matching confirmation email and Calendar range have been checked for duplicates."),
    tripId: z.string().optional(),
    confirm: z.boolean(),
...
server.tool("telegram_health", "Verify that the configured Telegram bot and destination chat are reachable. Makes read-only getMe and getChat requests and does not send a message.", {}, async () => result(await probeTelegramConnection()));
server.tool("telegram_preview_push", "Preview a relevant project notification before sending it to the configured Telegram chat. Use concise plain text and never include credentials, cookies, payment details, or raw private data.", {
    message: z.string().min(1).max(4096),
    title: z.string().min(1).max(160).optional(),
    silent: z.boolean().optional(),
    messageThreadId: z.number().int().positive().optional(),
}, async (input) => result(previewTelegramPush(input)));
server.tool("telegram_send_push", "Send a relevant, concise project notification to the Telegram chat configured in TELEGRAM_CHAT_ID. The project has standing send authorization, so no per-message confirmation is required when enabled in config. Preview first. Never send credentials, cookies, payment details, or unreviewed private data.", {
    message: z.string().min(1).max(4096),
    title: z.string().min(1).max(160).optional(),
    silent: z.boolean().optional(),
    messageThreadId: z.number().int().positive().optional(),
