...
server.tool("travel_health", "Check local travel store and connector readiness.", {}, async () => result({
    ...
    mapsApiKeyPresent: Boolean(process.env.GOOGLE_MAPS_GROUNDING_API_KEY),
}));
...
server.tool("travel_get_place", "Get one local place.", { placeId: z.string() }, async ({ placeId }) => result(getPlace(placeId)));
