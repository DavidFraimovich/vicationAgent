...
export function findPlaceByUrl(tripId, url) {
    if (!url)
        return undefined;
    const row = getDb().prepare("SELECT * FROM places WHERE trip_id = ? AND google_maps_url = ? LIMIT 1").get(tripId, url);
...
export function getPlace(id) {
    const row = getDb().prepare("SELECT * FROM places WHERE id = ?").get(id);
    return row ? normalizePlace(row) : undefined;
}
...
export function setPlaceStatus(input) {
    ...
    return upsertPlace({
        ...
        planningStatus: input.planningStatus ?? current.planningStatus,
