...
import { assertCalendarEventIsPlainText, calendarAttendeesWithoutPersonalInvite, normalizeCalendarDescription, previewCalendarEvent, } from "../src/google-calendar.js";
...
test("calendar writes remove only the personal invite and reject stored HTML", () => {
    assert.deepEqual(calendarAttendeesWithoutPersonalInvite([
    ...
    assert.doesNotThrow(() => assertCalendarEventIsPlainText({
        description: "Google Maps:\nhttps://maps.example/place",
        attendees: [],
    }, "Google Maps:\nhttps://maps.example/place"));
    assert.throws(() => assertCalendarEventIsPlainText({
...
test("calendar commands and configuration point to the authoritative policy", () => {
    ...
    const policyPath = ".agents/skills/travel-planner/references/calendar-policy.md";
    ...
    const config = fs.readFileSync(path.join(projectRoot, "config/travel-agent.yaml"), "utf8");
    ...
    for (const file of calendarInstructionEntryPoints) {
        ...
        assert.match(content, /calendar-policy\.md|CALENDAR_POLICY_FILE/, `${file} must reference the authoritative Calendar policy`);
    }
});
test("Airbnb and check-in lodging items are provider-managed calendar entries", () => {
    ...
    assert.equal(isLikelyLodgingCalendarEvent({
...
test("plan seeding is idempotent and trip-place links stay scoped", { concurrency: false }, async () => {
    ...
    const { closeDb, getDb, listItinerary, listLodging, listPlaces, upsertPlace, } = await import("../src/db.js");
    ...
    try {
        ...
        assert.equal(listPlaces({ tripId: "dolomites-2026", query: "Library-only", limit: 10 }).length, 0);
        upsertPlace({
            id: "library-only",
            tripId: "dolomites-2026",
            name: "Library-only place",
        ...
        assert.equal(listPlaces({ tripId: "dolomites-2026", query: "Library-only", limit: 10 }).length, 1);
        assert.equal(getDb().prepare("PRAGMA quick_check").get()?.quick_check, "ok");
    }
    finally {
        closeDb();
        if (oldDb === undefined)
            delete process.env.TRAVEL_AGENT_DB;
        else
            process.env.TRAVEL_AGENT_DB = oldDb;
        loadConfig(true);
